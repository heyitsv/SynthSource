function AudioSynthView() {
	...
	var fnCreateKeyboard = function(keyboardElement) {
		...
		for(var i=-1;i<=1;i++) {
			for(var n in notes) {
				if(n[2]!='b') {
					...
					thisKey.addEventListener(evtListener[0], (function(_temp) { return function() { 
               if (window.isRecording){
                  window.latestRec["events"].push([new Date().getTime(), _temp + "|keydown"]);
              }
            
             fnPlayKeyboard({keyCode:_temp}); } })(reverseLookup[n + ',' + i]));
       ...
       n = keysPressed.length; while(n--) { 
            if (window.isRecording){
              window.latestRec["events"].push([new Date().getTime(), keysPressed[n] + "|keup"]);
             }
	...
	var fnPlayKeyboard = window.fnPlayKeyboard = function(e) {
    ...
    keysPressed.push(e.keyCode);
		...
		if(keyboard[e.keyCode]) {
			if(visualKeyboard[keyboard[e.keyCode]]) {
				visualKeyboard[keyboard[e.keyCode]].style.backgroundColor = 'rgba(105, 164, 219, 0.8)';
	...
	var fnRemoveKeyBinding =  window.fnRemoveKeyBinding = function(e) {
